...
class DeviceAvailability {
    ...
    async getAllPingableDevices() {
        return (await this.zigbee.getAllClients()).filter((d) => this.isPingable(d));
    }
    ...
    async stop() {
        (await this.zigbee.getAllClients()).forEach((device) => this.publishAvailability(device.ieeeAddr, false));
    }
    ...
    async onZigbeeMessage(message, device, mappedDevice) {
        if (device && this.isPingable((await this.zigbee.getDevice(device.ieeeAddr)))) {
